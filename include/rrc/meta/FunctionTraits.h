...
namespace rrc {
    namespace meta {
         ...
         */
        template<typename T> class FunctionTraits;

        template<typename Res, typename ...Args> class FunctionTraits<std::function<Res(Args...)>> {
        public:
        ...
        template<typename Res, typename ...Args> class FunctionTraits<Res(*)(Args...)> {
        public:
            static const size_t count = sizeof...(Args);
            using Result = Res;
            template<size_t i> using Arg = typename std::tuple_element<i, std::tuple<Args...>>::type;
