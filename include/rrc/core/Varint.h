...
namespace rrc {
    ...
    private:
        ...
        inline static size_t encodeImplementation(Value value, uint8_t* output, IterationTag) {
            using Tag = IterationTraits<N - 1>;
            return (value > 127)
        ...
        inline static size_t encodeImplementation(Value value, uint8_t* output, FinalIterationTag) {
            return (output[sizeof(Value) + 1] = ((uint8_t) value & 127), sizeof(Value) + 1);
        };

        template<class Value, size_t N = sizeof(Value) + 1>
        inline static Value decodeImplementation(uint8_t* input, Value acc, IterationTag) {
            using Tag = IterationTraits<N - 1>;
        ...
        template<class Value, size_t N>
        inline static Value decodeImplementation(uint8_t* input, Value acc, FinalIterationTag) {
    ...
    public:
        ...
        static size_t encode(Value value, uint8_t* output) {
            return Varint::encodeImplementation(value, output, IterationTag());
        }

        template<class Value = uint64_t>
        static Value decode(uint8_t* input) {
            return Varint::decodeImplementation(input, 0, IterationTag());
        }
    };
}